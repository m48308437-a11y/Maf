...
const JWT_SECRET = process.env.JWT_SECRET || "mfa-change-this-secret";

const dbPath = process.env.DB_PATH || path.join(__dirname, "mfa.db");
  ...
  try {
    ...
    if (!username || password.length < 6) {
      return res.status(400).json({
        error: "نام کاربری و رمز عبور حداقل معتبر را وارد کنید"
      });
    }
...
app.get("/api/anime/:id", (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "خطا در دریافت اطلاعات انیمه"
    });
  }
});
  ...
  (req, res) => {
    try {
      ...
      const result = db
        .prepare(`
          INSERT INTO anime
          (title, jp_title, year, studio, genres, status, score, synopsis, poster)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `)
        .run(
          String(title).trim(),
  ...
  (req, res) => {
    try {
      ...
      const updated = {
        ...
        jp_title: req.body.jp_title ?? current.jp_title,
        year: req.body.year ?? current.year,
        studio: req.body.studio ?? current.studio,
        genres: req.body.genres ?? current.genres,
        status: req.body.status ?? current.status,
        score: req.body.score ?? current.score,
        synopsis: req.body.synopsis ?? current.synopsis,
        poster: req.body.poster ?? current.poster
      };
...
app.delete(
  ...
  (req, res) => {
    try {
      ...
      db.prepare("DELETE FROM comments WHERE anime_id = ?").run(id);
...
app.post("/api/anime/:id/comments", authRequired, (req, res) => {
  try {
